package io.github.norbertweb.bluebird.media
...
object MediaLibraryRepository {
    ...
    // MediaStore is well known to carry duplicate rows for the same physical
    // file — most commonly after an SD card gets remounted under a new
    // volume ID, or after a rescan leaves a stale row alongside a fresh one.
    // Both rows point at the same absolute path but have different _IDs, so
    // they show up as two separate library entries for one real file. Since
    // MediaStore.Audio.Media.EXTERNAL_CONTENT_URI spans every mounted volume
    // (internal + all SD cards) since Android 10, this is the case that was
    // producing duplicate entries, especially for Music libraries stored on
    // removable storage. Fix: collapse by absolute path, keeping whichever
    // row was modified most recently.
    private fun List<ScannedTrack>.dedupedByPath(): List<ScannedTrack> {
        val byPath = LinkedHashMap<String, ScannedTrack>()
        for (t in this) {
            val key = t.file?.absolutePath ?: t.contentUri.toString()
            val existing = byPath[key]
            if (existing == null || t.dateModified > existing.dateModified) byPath[key] = t
        }
